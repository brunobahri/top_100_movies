...
def extract_movie_details(movie_link):
    ...
    if movie_soup is not None:
        ...
        if movie_data is not None:
            ...
            title = (
                movie_data.find("h1", attrs={"data-testid": "hero__pageTitle"})
                .find("span")
                .getText()
            )
        ...
        rating = (
            movie_soup.find("span", attrs={"sc-bde20123-1 cMEQkK"}).get_text()
            if movie_soup.find("span", attrs={"sc-bde20123-1 cMEQkK"})
            else None
        )
        # plot é o texto de sinopse do filme
        plot_text = (
            movie_soup.find("span", attrs={"class": "sc-466bb6c-2 chnFO"})
            .get_text()
            .strip()
            if movie_soup.find("span", attrs={"class": "sc-466bb6c-2 chnFO"})
            else None
        )
